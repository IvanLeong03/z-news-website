        ...
        {/* Input container (fixed height) */}
            <form
                ...
                className={`absolute right-0 top-1/2 -translate-y-1/2 ${open ? expandedW : collapsedW} transition-[width] duration-300 ease-in-out
                 flex items-center rounded-xl border border-[var(--color-line-darkgrey)] bg-[var(--color-gs-black)] ${
                open ? "shadow-md" : ""
                }`}
            >
        ...
        {/* Popdown – expands *below* the input */}
            <div
                className={`absolute right-0 top-full mt-5 ${open ? expandedW : collapsedW} transition-[width] duration-300 ease-in-out
                  bg-[var(--color-gs-black)] border border-gray-800 rounded-xl shadow-lg overflow-hidden origin-top 
                    ...
                    {loading ? (
                        ...
                        {trending.map((a, i) => (
                            ...
                            <button
                                ...
                                className={`w-full text-left px-3 py-2 text-sm hover:bg-gray-400 ${
                                highlighted === i ? "bg-gray-400" : ""
                                }`}
                                title={a.title}
                            >
                                {a.title}
