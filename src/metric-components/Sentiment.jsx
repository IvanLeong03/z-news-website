import React from 'react';
import { useState } from 'react';
import { useLanguage } from "../context/LanguageContext";


function Sentiment({sentiment=0}) {
const { language } = useLanguage();
  // Clamp the score to stay between -1 and 1
  const clampedScore = Math.max(-1, Math.min(sentiment, 1));
  // Convert to percentage (0% to 100%) with 0 at center
  const percentage = ((clampedScore + 1) / 2) * 100;

  return (
    <div className="w-full mb-4">
      <div className="relative h-1 bg-gradient-to-l from-[var(--color-dark-turquoise)] to-[var(--color-light-turquoise)] rounded-full">
        <div
          className="absolute top-0 -mt-1 w-0.5 h-4 bg-black"
          style={{ left: `${percentage}%`, transform: "translateX(-50%)" }}
        />
        {/* optional tick markers */}
        <div className="absolute left-1 top-2 text-xs text-gray-600">-1</div>
        <div className="absolute left-1/2 top-2 transform -translate-x-1/2 text-xs text-gray-600">0</div>
        <div className="absolute right-1 top-2 text-xs text-gray-600">+1</div>
      </div>
      <p className="text-xs text-center mt-4">{language === 'zh-Hant'? "文本情感分析" : language === "zh-Hans" ? "文本情感分析" : "Sentiment"}: {sentiment.toFixed(2)}</p>
    </div>
  );
}

export default Sentiment;

